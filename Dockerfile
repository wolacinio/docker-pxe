...
ENV ip=172.17.0.2
ENV range_from=172.17.0.2
ENV range_to=172.17.255.254
ENV broadcast=172.17.255.255
USER root:root
SHELL ["/bin/bash", "-c"]
WORKDIR /root
RUN apt-get update && apt-get install -y -o "DPkg::Options::=--force-confold" -o "DPkg::Options::=--force-confdef" systemd-sysv rsyslog net-tools
